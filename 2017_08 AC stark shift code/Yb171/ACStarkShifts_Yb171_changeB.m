function [AC_diff_1, AC_diff_end, abs_shift_up, abs_shift_down] = ACStarkShifts_Yb171_changeB( B )
...
% B is now given as an input to the function
...
tru = [1/9 1/9 0 1/9 1/18 0 1/18 0 1/6 2/9 1/6 0];
...
fg328 = c/lambda32;
tunevec = [-200,200]*1e9;           % range of detuning...
...
for jj=1:nd
    ...
    Eacd = Rabidr.^2./(4*deltadown) + Rabidb.^2./(4*deltadown);
    Eac_dtot(jj) = sum(Eacd);

    % Stark shifts for up
    Eacu = Rabiur.^2./(4*deltaup) + Rabiub.^2./(4*deltaup);
    Eac_utot(jj) = sum(Eacu);
end    
...
AC_diff_1 = (Eac_dtot(1) - Eac_utot(1))/(2*pi);
AC_diff_end = (Eac_dtot(end) - Eac_utot(end))/(2*pi);
abs_shift_down = Eac_dtot(1)/(2*pi);
abs_shift_up = Eac_utot(1)/(2*pi);
